import { DNodes } from '../lib/draw';
import { map, net } from './singletons';
function createCanvas(width = 1900, height = 600) {
    let canvs = document.getElementById('post');
...
function _renderCurve(el) {
    ...
    el.attributes.curveFrom.forEach(p => {
        ...
        post.bezierCurveTo(p[1].x, p[1].y, el.attributes.pointFrom.x, el.attributes.pointFrom.y, el.center.x, el.center.y);
        post.stroke();
    });
    el.attributes.curveTo.forEach(p => {
        post.moveTo(el.center.x, el.center.y);
        post.bezierCurveTo(el.attributes.pointTo.x, el.attributes.pointTo.y, p[0].x, p[0].y, p[1].x, p[1].y);
        post.stroke();
    });
    post.restore();
}
